...
export default function DashboardPage() {
  ...
  useEffect(() => {
    if (status === "unauthenticated") {
      router.push("/")
    } else if (status === "authenticated" && user?.role?.toLowerCase() !== (params.role as string).toLowerCase()) {
      // In next-auth v5 role might be uppercase or lowercase depending on how we seeded
      // Let's be case-insensitive
      // router.push("/") 
      // Actually, if we are in [role] and role doesn't match, we might want to redirect
    }
  }, [status, user, params.role, router])
